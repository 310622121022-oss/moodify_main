import type { Metadata } from 'next';
import { getSeoMetadata } from '@/lib/seo-service';

export const dynamic = 'force-dynamic';

const defaultMetadata: Metadata = {
  title: 'Name the Moment | MoodLift',
  description: 'Name the current moment to practice mindfulness and increase awareness of your present experience.',
  keywords: 'name the moment, mindfulness, awareness, present moment, meditation',
  openGraph: {
    title: 'Name the Moment | MoodLift',
    description: 'Name the current moment to practice mindfulness and increase awareness of your present experience.',
    url: 'https://moodlift.com/games/name-the-moment',
    siteName: 'MoodLift',
    images: [
      {
        url: 'https://moodlift.com/images/og-name-the-moment.jpg',
        width: 1200,
        height: 630,
        alt: 'Name the Moment - MoodLift',
      },
    ],
    locale: 'en_US',
    type: 'website',
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Name the Moment | MoodLift',
    description: 'Name the current moment to practice mindfulness and increase awareness of your present experience.',
    images: ['https://moodlift.com/images/og-name-the-moment.jpg'],
  },
  alternates: {
    canonical: '/games/name-the-moment',
  },
};

export async function generateMetadata(): Promise<Metadata> {
  try {
    const seo = await getSeoMetadata('/games/name-the-moment');
    if (!seo) return defaultMetadata;

    const ogImages = seo.og_image ? [{ url: seo.og_image, alt: seo.title }] : defaultMetadata.openGraph?.images;

    return {
      title: seo.title || defaultMetadata.title,
      description: seo.description || defaultMetadata.description,
      keywords: seo.keywords || defaultMetadata.keywords,
      metadataBase: new URL('https://moodlift.com'),
      alternates: defaultMetadata.alternates,
      openGraph: {
        title: seo.title || defaultMetadata.openGraph?.title,
        description: seo.description || defaultMetadata.openGraph?.description,
        url: 'https://moodlift.com/games/name-the-moment',
        siteName: defaultMetadata.openGraph?.siteName,
        images: ogImages as any,
        locale: defaultMetadata.openGraph?.locale,
      },
      twitter: {
        title: seo.title || defaultMetadata.twitter?.title,
        description: seo.description || defaultMetadata.twitter?.description,
        images: seo.og_image ? [seo.og_image] : defaultMetadata.twitter?.images,
      },
    } as Metadata;
  } catch (err) {
    console.error('Error generating metadata:', err);
    return defaultMetadata;
  }
}

export default function GameLayout({
  children,
}: {
  children: React.ReactNode;
}) {
  return <>{children}</>;
}